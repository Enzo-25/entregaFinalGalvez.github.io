...
function cargarEventosDesdeJSON() {
    fetch('./eventos.josn') // Ruta al archivo JSON
      .then(response => response.json())
      .then(data => {
        // Aquí puedes actualizar tus datos de eventos con los datos cargados desde el archivo JSON
        eventos = data;
        // Luego, puedes volver a renderizar la lista de eventos en tu página
        renderizarListaDeEventos();
      })
      .catch(error => {
        console.error('Error al cargar eventos desde el archivo JSON:', error);
      });
}
...
function mostrarEnCarrito(){
    ...
    carritoDeCompras.forEach(function(pedido) {
        if(!document.getElementById("texto-pedido" + pedido.id)){
            ...
            textoPedido.textContent = pedido.entradas + "x entradas para " + pedido.evento + " por S/." + pedido.precio + ".  ";
    ...
    textoTotal.textContent = "TOTAL = S/." + precioTotal;
...
botonRegistro.addEventListener("click", (e) => {
    ...
    if(usuario.length == 0 || pass.length == 0){
        ...
        mensajeIncorrecto.innerText = "Tienes que completar todos los campos para registrarte.";
...
botonConfirmarCarrito.addEventListener("click", function(e){
    ...
    if(!JSON.parse(localStorage.getItem("usuarioActivo")).usuario){
        ...
        mensajeCarrito.innerText = "Tienes que iniciar sesión para confirmar el carrito de compras.";
...
botonConfirmarCompra.addEventListener("click", function(e){
    ...
    if(datosDePago.some(item => 0 == item.length)){
        ...
        mensajePago.innerText = "Tienes que completar todos los campos para confirmar la compra.";
